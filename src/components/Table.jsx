...
export default function Table() {
  ...
  useEffect(() => {
    gitApiData(); // call function git api
  }, []);

  // search data by button search by name
  const filterData = dataTable.filter((item) =>
    item.name.toLowerCase().includes(searchQuery.toLowerCase()),
  );

  //Delete element by button Delete by tow function
          ...
          {filterData.map((item, i) => (
            <tr key={item.id}>
              ...
              <td>
                {item.name}
                <br />
                <p className=" opacity-50"> {item.address.zipcode}</p>
              </td>
              <td>{item.company.catchPhrase}</td>
              <td>{item.website} </td>
              {item.address.geo.lng >= 0 ? (
                <td>
                  <p className=" text-success">
                    {item.address.geo.lng}
                    <br /> <p className=" text-dark">CAD</p>{" "}
                  </p>
                </td>
              ) : (
                <td>
